...
public class TestsSupport {
	...
	public static boolean isCorrect(String filename, String results) {
        ...
        String officialResults = getExpectedResults(filename); 
	...
	private static String getExpectedResults(String filename) {
        ...
        try {
            ...
            throw new AssertionError("Expected results for " + filename + " not found");
        }
		return officialResults;
	}
	
	public static void assertCorrect(String filename, String results) {
		officialUseIgnore(filename, results);
        String officialResults = getExpectedResults(filename); 
         
        if (removeBlanks(results).equals(removeBlanks(officialResults))) {
            return;
        }
        
        assertEquals(officialResults, results);
	}
